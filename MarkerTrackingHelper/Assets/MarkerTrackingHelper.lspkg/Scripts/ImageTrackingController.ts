...
export class ImageTrackingController extends BaseScriptComponent {
  ...
  onAwake(): void {
    ...
    if (mainCamera) {
       ...
       * The native getAspectRatio() throws "Cannot get aspect ratio, missing provider"
       * when the MarkerTrackingComponent's marker has no texture provider assigned
       * (e.g. the prefab was instantiated without wiring a marker image in the
       * Inspector). We fall back to 1.0 (square) so onAwake doesn't throw — that
       * would prevent every downstream component on the SceneObject from being
       * awake'd, cascading "Component is not yet awake" errors. */
      try {
        ...
        this.height = 0;
        this.logger.debug(
          `WARN: marker asset has no provider (${e}); using aspectRatio=1.0, height=0. Assign a marker image on the MarkerTrackingComponent in the Inspector to enable scaling.`
        );
      }
