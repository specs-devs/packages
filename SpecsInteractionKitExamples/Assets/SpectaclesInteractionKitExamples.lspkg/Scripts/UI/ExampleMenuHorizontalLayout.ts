...
// finish defining COLORS before RoundedRectangleVisual reads it. Must stay the FIRST import.
...
/**
 * Programmatic example: Menu Horizontal Layout
 *
 * Horizontal tab bar — Home / Gallery / Settings — each with a scrollable grid.
 * All structure is built at runtime in buildUI(). Adjust inputs and press Play to rebuild.
 */
@component
export class ExampleMenuHorizontalLayout extends BaseScriptComponent {
  ...
  @ui.label('<span style="color: #94A3B8; font-size: 11px;">const menu = so.getComponent("ExampleMenuHorizontalLayout")</span>')
  ...
  panelTitle: string = "Menu Horizontal"
  @ui.group_end
  ...
  frameInnerSize: vec2 = new vec2(40, 28)
  ...
  private buildUI(): void {
    ...
    const OUTER_W = PW - 2
    const OUTER_H = PH - 2
    const GRID_W = OUTER_W - 2 * this.contentInset.x
    const CONTENT_H = OUTER_H - TITLE_H - TABBAR_H - 2 * COL_GAP - 2 * this.contentInset.y
    ...
    const panelContent = this.scenePanel(
      root, "MenuHorizontalPanel", PW, PH,
    ...
    this.flexChild(outer, {w: GRID_W, h: TITLE_H}, (t) => {
      this.label(t, this.panelTitle, GRID_W, TITLE_H, {textSize: this.titleTextSize, align: "left", fontWeight: "bold"})
    })

    type TabPair = {activeObj: SceneObject; inactiveObj: SceneObject}
    const tabPairs: TabPair[] = []
    const tabW = (GRID_W - 2 * TAB_GAP) / 3
    this.flexChild(outer, {w: GRID_W, h: TABBAR_H}, (tabBarSlot) => {
      const tabRow = this.flexRow(tabBarSlot, GRID_W, TABBAR_H, {
        gap: TAB_GAP, justify: FlexJustify.Start, align: FlexAlign.Center
      })
      ...
      resolvedTabNames.forEach((name, index) => {
        ...
        this.flexChild(tabRow, {w: tabW, h: 4}, (tabObj) => {
          // Primary version — shown when this tab is active
          const activeObj = this.obj(tabObj, "TabActive")
          const activeBtn = this.btn(activeObj, "Primary", "Rectangle", tabW, 4)
          ...
          const inactiveBtn = this.btn(inactiveObj, "Secondary", "Rectangle", tabW, 4)
          ...
          const onTap = () => {
            ...
            print(`Menu Horizontal: tab "${name}" selected`)
          }
