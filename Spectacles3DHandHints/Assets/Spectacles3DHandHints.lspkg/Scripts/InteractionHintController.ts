...
export class InteractionHintController extends BaseScriptComponent {
  ...
  private onUpdate() {
    ...
    if (
      ...
      const indexTipPos = this.leftIndexTip.getTransform().getWorldPosition();
      const thumbTipPos = this.leftThumbTip.getTransform().getWorldPosition();
      ...
      if (this.cursor.enabled == true) {
        ...
        this.cursorMaterial.mainPass.circleSquishScale = MathUtils.remap(
          clampedD,
          1,
          12,
          0.5,
    ...
    if (
      ...
      const indexTipPos = this.rightIndexTip.getTransform().getWorldPosition();
      const thumbTipPos = this.rightThumbTip.getTransform().getWorldPosition();
    ...
    switch (clipInfo.handMode) {
      case 0:
        ...
        if (!isNull(this.left_outlineFadeTween)) {
          ...
          mainGroup.remove(this.left_outlineFadeTween);
        }
      ...
      case 1:
        ...
        if (!isNull(this.right_outlineFadeTween)) {
          ...
          mainGroup.remove(this.right_outlineFadeTween);
        }
    ...
    switch (this.currentHandMode) {
      case 0:
        ...
        if (!isNull(this.current_tween)) {
          ...
          mainGroup.remove(this.current_tween);
        }
