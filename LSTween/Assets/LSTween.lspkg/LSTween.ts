...
import { mainGroup } from "./TweenJS/mainGroup";
import now from "./TweenJS/Now";
...
export class LSTween {
  ...
  public static rawTween(time: number): Tween {
    const tween = new Tween({ t: 0 }, mainGroup).to({ t: 1 }).duration(time);
    return tween;
  }
  /**
   *
  ...
  public static alphaTo(material: Material, to: number, time: number) {
    ...
    const tween = new Tween({ a: initColor }, mainGroup)
      .to({ a: toClamp }, time)
      .onUpdate((object) => {
  ...
  public static alphaFromTo(
    ...
    const toClamp = MathUtils.clamp(to, 0.0, 1.0);
    const tween = new Tween({ a: fromClamp }, mainGroup)
      .to({ a: toClamp }, time)
      .onUpdate((object) => {
        const color = mainPass.baseColor;
        color.a = object.a;
  ...
  public static colorTo(material: Material, to: vec4, time: number) {
    ...
    const tween = new Tween({ t: 0.0 }, mainGroup)
      .to({ t: 1.0 }, time)
      .onUpdate((object) => {
  ...
  public static enableChange(
    ...
    const tween = new Tween(undefined, mainGroup)
      .to(undefined, time)
      .onComplete(() => {
  ...
  public static shaderFloatPropertyTo(
    pass: Pass,
    property: string,
    to: number,
    time: number
  ...
  public static shaderVec3PropertyFromTo(
    pass: Pass,
    property: string,
    from: vec3,
    to: vec3,
  ...
  public static valueTo(value: number, to: number, time: number) {
    const initVal = value;
    const tween = new Tween({ v: initVal }, mainGroup)
      .to({ v: to }, time)
      .onUpdate((object) => {
        value = object.v;
      });
  ...
  public static rotateToLocalInDegrees(
    transform: Transform,
    to_in_degrees: vec3,
    time: number
  ) {
    ...
    const tween = new Tween({ t: 0.0 }, mainGroup)
      .to({ t: 1.0 }, time)
      .onUpdate((object) => {
        const lerpedRot_in_degrees = vec3.lerp(
          initRot_in_degrees,
          to_in_degrees,
          object.t
        );
        transform.setLocalRotation(
          quat.fromEulerVec(
            lerpedRot_in_degrees.uniformScale(MathUtils.DegToRad)
          )
        );
      });
    return tween;
  ...
  public static rotateFromToLocalInDegrees(
    transform: Transform,
    from_in_degrees: vec3,
    to_in_degrees: vec3,
    time: number
  ) {
    // Set initial local rotation immediately
    transform.setLocalRotation(
      quat.fromEulerVec(from_in_degrees.uniformScale(MathUtils.DegToRad))
    );
  ...
  public static rotateToWorldInDegrees(
    transform: Transform,
    to_in_degrees: vec3,
    time: number
  ) {
  ...
  public static rotateFromToWorldInDegrees(
    transform: Transform,
    from_in_degrees: vec3,
    to_in_degrees: vec3,
    time: number
  ...
  public static blendShapeValueTo(
  ...
  public static blendShapeValueFromTo(
    renderMeshVisual: RenderMeshVisual,
    blendShapeName,
    from: number,
    to: number,
    ...
    const tween = new Tween({ v: 0.0 }, mainGroup)
      .to({ v: 1.0 }, time)
      .onUpdate((object) => {
