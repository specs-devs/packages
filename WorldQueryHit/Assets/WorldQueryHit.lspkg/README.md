...
A comprehensive surface detection and interaction system for Specs that integrates with the Specs Interaction Kit to enable real-world surface placement and spatial awareness. This package provides ray-casting hit testing against detected surfaces in the physical environment, allowing users to place virtual objects on tables, walls, floors, and other real-world surfaces. With support for surface classification, filtering, and continuous tracking, WorldQueryHit transforms Specs into a powerful spatial computing platform.

## Features

- **Real-World Surface Detection** - Ray-cast against detected environmental surfaces
- **SIK Integration** - Seamless integration with Specs Interaction Kit for hand/controller input
