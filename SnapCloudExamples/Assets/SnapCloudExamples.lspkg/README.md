...
- **LeaderboardRowInstantiator.ts**: Instantiates a pool of row prefabs under its scene object, positions them (startPosition + step x index), and binds leaderboard data via each row's bind(entry). Designed to sit inside Specs UI Kit's ScrollWindow.
