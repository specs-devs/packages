...
class WebSocketPolyfill {
    ...
    _initializeConnection() {
        try {
            this._lensWebSocket = supabaseModule.createRealtimeConnection(this.url, this.protocols);
            if (!this._lensWebSocket) {
                throw new Error("Failed to create WebSocket instance from SupabaseModule");
            }
...
class Request {
    ...
    toLensStudioRequest() {
        ...
        // Native Lens Studio Request does not support HEAD (only GET/POST/PUT/DELETE/PATCH).
        // Convert HEAD → GET so the request reaches the server. The JS layer still checks
        // this.method === 'HEAD' to skip body parsing, and PostgREST still returns the
        // count in the Content-Range response header when Prefer: count=exact is set.
        const lsHttpMethod = globalThis.RemoteServiceHttpRequest?.HttpRequestMethod;
        const isHeadSupported = lsHttpMethod != null && 'Head' in lsHttpMethod;
        const effectiveMethod = (this._method === 'HEAD' && !isHeadSupported) ? 'GET' : this._method;
        // When falling back HEAD → GET, cap the response to 1 row so the server doesn't
        // send the full body. PostgREST still populates Content-Range with the total count.
        // Skip if the caller already set a Range header (e.g. explicit pagination).
        if (this._method === 'HEAD' && !isHeadSupported && !this._headers.has('range')) {
            lsHeaders['range'] = '0-0';
        }
        const options = {
