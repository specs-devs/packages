/**
 * Specs Inc. 2026
 * Navigation utility functions for geographic and spatial calculations. Provides Haversine distance
 * computation between lat/long coordinates, bearing calculation for directional navigation, scene
 * hierarchy traversal helpers, quaternion-to-Euler conversion, angle normalization to [-180, 180]
 * range, and string-to-number parsing for coordinate input validation.
 */
...
import {mapValue} from "SpectaclesInteractionKit.lspkg/Utils/mathUtils"
...
export function getOffsetForLocation(
  ...
  const tileOffsetForLocation = mapModule.longLatToImageRatio(latitude, longitude, initialLocation)
...
export function calculateZoomOffset(zoomLevel: number): number {
  return mapValue(zoomLevel, 8, 21, -11, 2)
}
