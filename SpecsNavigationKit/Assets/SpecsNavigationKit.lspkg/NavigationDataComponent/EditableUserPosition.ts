...
export class EditableUserPosition implements UserPosition {
  ...
  private locationServiceConfigured = false

  public initializeGeoLocationUpdates(accuracy: GeoLocationAccuracy, updateFrequency: number): void {
    this.updateFrequency = updateFrequency
    // Subscribe to native orientation updates and set accuracy only once per
    // component lifetime. LS's native GPS layer logs
    // "initializeGpsStartTime: Function called in an incorrect context
    // (gps time initialized: true, has incoming requests: false)" when its
    // internal GPS start-time bookkeeping is touched redundantly; guarding
    // against duplicate subscription here avoids at least the application-side
    // contribution to that path. (The warning can still appear on a cold GPS
    // session because it originates from LS native code; it is not fatal.)
    if (!this.locationServiceConfigured) {
      ...
      this.locationServiceConfigured = true
    }
