...
A comprehensive motion capture integration package for streaming Sony mocopi sensor data to Snap Specs AR glasses. This package provides WebSocket client connectivity, automatic bone mapping, coordinate system conversion, surface placement integration, and real-time avatar animation with smooth interpolation for full-body motion tracking experiences.
...
#### Option B: Hybrid Cloud Setup (Recommended for Specs Device)

Use this when Specs has difficulty connecting to local WebSocket servers. Uses a cloud relay for reliable WebSocket connections.

**Architecture:** mocopi app (UDP) → Your PC (forwarder) → Railway (cloud relay) → Specs
...
**Benefits:**
- Reliable WebSocket connection for Specs
- No local network configuration needed on Specs
- Works across different networks

**Note:** Your PC and mocopi app must still be on the same local network. Only the Specs → Server connection is through the cloud.
...
## Deploying to Specs Device

<img src="README-ref/screenrecording_specs.gif" width="600" alt="mocopi on Spectacles">

> **Important**: You must enable **Experimental Mode** in Lens Studio to build and deploy to Specs. Go to **Project Settings → General → Enable Experimental Mode**.

### Recommended Setup: Hybrid Cloud Architecture

For Specs on-device testing, use the **Hybrid Cloud Setup (Option B)** described above. This provides the most reliable connection:
...
3. **Deploy to Specs** - the WebSocket URL is already configured:
   ```typescript
   serverUrl: "wss://mocopy-railway-service-production.up.railway.app"
   ```
...
- Specs → Cloud connection uses public WebSocket (no local network issues)

### Alternative: All Local Setup

If Specs can connect to your local network:
